...
dojo.declare("betterform.ui.common.GrowlAlert", null, {
	...
	constructor : function(params) {
		// Combine passed parameters with default params before creating
		// template
...
//		}
	...
	handleInvalid : function(id, action) {
		...
		// console.debug("betterform.ui.common.GrowlAlert.invalid [id:" + id , "
		// action: " + action + " position: " + this.position + " stack: " +
		// this.stack +"]");
		if (alertGrowl == undefined && alert != undefined) {
			if (label != undefined) {
				alertGrowl = new betterform.ui.common._Growl({
					id : id + "-Growl-alert",
					control : control,
					controlLabel : label.innerHTML,
		...
		if (alert != undefined) {
			...
			// if(action=="onFocus" || ((action=="onBlur" ||
			// action=="submitError") && controlValue != '')){
			if (action == "onFocus"
					|| (action == "onBlur" && controlValue != '')) {
				alertGrowl.show();
			} else if (action == "applyChanges"
					&& ((controlValue != '' && controlValue != '00:00:00') || dojo.hasClass(control.domNode,
							"xsdBoolean"))) {
				// 00:00:00 check for date-time 
				alertGrowl.show();
