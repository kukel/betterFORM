...
-->
<xsl:stylesheet version="2.0" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xforms="http://www.w3.org/2002/xforms" xmlns:bf="http://betterform.sourceforge.net/xforms" xmlns:bfc="http://betterform.sourceforge.net/xforms/controls" exclude-result-prefixes="xhtml xforms bf" xpath-default-namespace="http://www.w3.org/1999/xhtml">
	<!-- ####################################################################################################### -->
	...
	<xsl:template match="xforms:repeat[@appearance='compact']" name="compact-repeat">
		...
		<table repeatId="{$repeat-id}" jsId="{$repeat-id}" class="{$repeat-classes}" dojoType="betterform.ui.container.Repeat" appearance="compact" border="0" cellpadding="0" cellspacing="0">
			...
			<xsl:for-each select="xforms:group[@appearance='repeated']">
				...
				<tr repeatItemId="{$id}" class="{$repeat-item-classes}" dojoType="betterform.ui.container.RepeatItem" appearance="compact">
	...
	<xsl:template match="xforms:input|xforms:output|xforms:range|xforms:secret|xforms:select|xforms:select1|xforms:textarea|xforms:upload" mode="repeated-compact-prototype" priority="10">
		...
		<xsl:variable name="htmlElem">
			<xsl:choose><xsl:when test="local-name()='output'">span</xsl:when>
		...
		<xsl:element name="{$htmlElem}">
			...
			<xsl:call-template name="copy-style-attribute" />
			<xsl:choose>
				<!-- RKU: Difference with previous, not by me??? -->
	...
	<xsl:template match="xforms:group" mode="repeated-compact-prototype" priority="10">
		...
		<xsl:element name="{$htmlElem}">
			...
			<xsl:apply-templates select="*[not(self::xforms:label or self::xforms:hint) ]" mode="repeated-compact-prototype" />
		</xsl:element>
	</xsl:template>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<div id="{$id}" class="{$control-classes} xfRepeated" unbound="true">
					<div id="{$id}-value" class="xfValue" appearance="{$appearance}" controlType="trigger" name="d_{$id}" navindex="" accesskey="" source="">
	...
	<xsl:template match="xforms:input|xforms:output|xforms:range|xforms:secret|xforms:select|xforms:select1|xforms:textarea|xforms:upload" mode="repeated-full-prototype" priority="20">
		<xsl:variable name="id" select="@id" />
		<xsl:variable name="control-classes">
			<xsl:call-template name="assemble-control-classes" />
		</xsl:variable>
	...
	<xsl:template match="xforms:group" mode="repeated-full-prototype" priority="10">
		<xsl:variable name="id" select="@id" />
		<xsl:variable name="appearance" select="@appearance" />
		<xsl:variable name="htmlElem">
			<xsl:choose>
	...
	<xsl:template match="xforms:switch" mode="repeated-full-prototype" priority="10">
		<xsl:variable name="switch-id" select="@id" />
		<xsl:variable name="switch-classes">
			<xsl:call-template name="assemble-compound-classes">
				<xsl:with-param name="appearance" select="@appearance" />
	...
	<xsl:template match="xforms:case[bf:data/@bf:selected='true']" mode="repeated-full-prototype" priority="10">
		<xsl:variable name="case-id" select="@id" />
		<xsl:variable name="case-classes" select="'xfCase xfSelectedCase'" />
		<div id="{$case-id}" class="{$case-classes}">
			<xsl:apply-templates select="*[not(self::xforms:label)]" mode="repeated-full-prototype" />
	...
	<!-- ### DE-SELECTED/NON-SELECTED CASE ### -->
	<xsl:template match="xforms:case" mode="repeated-full-prototype" priority="10">
		<!-- render only in scripted environment -->
		<xsl:variable name="case-id" select="@id" />
		<xsl:variable name="case-classes" select="'xfCase xfDeselectedCase'" />
		<div id="{$case-id}" class="{$case-classes}">
	...
	<xsl:template match="xforms:repeat" mode="repeated-full-prototype" priority="10">
		<xsl:variable name="id" select="@id" />
		<xsl:variable name="control-classes">
			<xsl:call-template name="assemble-control-classes" />
		</xsl:variable>
	...
	<xsl:template match="xhtml:*" mode="repeated-full-prototype" priority="1">
		<xsl:copy>
			<xsl:copy-of select="@*" />
			<xsl:apply-templates mode="repeated-full-prototype" />
		</xsl:copy>
	...
	<xsl:template match="xforms:repeat[@appearance='full']" name="full-repeat">
		...
		<div repeatId="{$repeat-id}" class="{$repeat-classes}" dojoType="betterform.ui.container.Repeat">
			...
			<xsl:for-each select="xforms:group[@appearance='repeated']">
				...
				<div repeatItemId="{$repeat-item-id}" class="{$repeat-item-classes}" dojoType="betterform.ui.container.RepeatItem" appearance="full" tabindex="0">
					<div class="legend">
						<xsl:choose>
							<xsl:when test="$group-label and xforms:label">
								<xsl:attribute name="id">
	...
	<xsl:template match="xforms:case[bf:data/@bf:selected='true']" name="selected-case">
		<xsl:variable name="case-id" select="@id" />
		<!-- RKU: Added author classes-->
		<xsl:variable name="author-classes">
			<xsl:call-template name="get-author-classes" />
		</xsl:variable>
		<xsl:variable name="case-classes" select="concat('xfCase xfSelectedCase', $author-classes)" />
		<div id="{$case-id}" class="{$case-classes}">
			<xsl:apply-templates select="*[not(self::xforms:label)]" />
		</div>
	</xsl:template>
	...
	<xsl:template match="xforms:case" name="deselected-case">
		...
		<xsl:variable name="case-classes" select="concat('xfCase xfDeselectedCase', $author-classes)" />
		<div id="{$case-id}" class="{$case-classes}">
			<xsl:apply-templates select="*[not(self::xforms:label)]" />
		</div>
	</xsl:template>
	<xsl:template match="xforms:switch[@appearance='dijit:AccordionContainer']">
		...
		<div id="{$switch-id}" class="{$switch-classes} xfSwitch" dojoType="dijit.layout.AccordionContainer" duration="200" style="float: left; margin-right: 30px; width: 400px; height: 300px; overflow: hidden">
	...
	<xsl:template match="xforms:switch[@appearance='dijit:TabContainer']">
		<xsl:variable name="switch-id" select="@id" />
		<!-- Why is this commented out? compared to mine -->
		<!--
			<xsl:variable name="switch-classes">
			<xsl:call-template name="assemble-compound-classes">
			<xsl:with-param name="appearance" select="@appearance"/>
		...
		<!-- RKU: switch-classes added here by me to -->
		<div id="{$switch-id}" class="xfSwitch bfTabContainer" dojoType="betterform.ui.container.TabSwitch">
			...
			<xsl:for-each select="xforms:case[./xforms:label]">
				...
				<!-- RKU: author-classes added here by me to -->
	...
	<xsl:template match="xforms:switch[@appearance='dijit:TitlePane']">
		...
		<div id="{$switch-id}" class="{$switch-classes}" style="width: 600px; height: 300px;">
			<xsl:for-each select="xforms:case[.//xforms:label]">
